...
def calculate_parameters(
    ...
    for bond, p in labels["Bonds"].items():
        # print(bond,p)
        if bond[0] in atoms_of_interest or bond[1] in atoms_of_interest:
    ...
    for key, p in labels["Angles"].items():
        if key[0] in atoms_of_interest or (key[1] in atoms_of_interest or key[2] in atoms_of_interest):
